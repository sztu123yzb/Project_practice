...
def down1():
    url = get_final_url()
    # url = "https://www.douyin.com/note/7393420246909996339"
    xuanze(url)
...
def shipin(biaoqian,s):
    if s == 200:
        # Parse the HTML content with BeautifulSoup
       #html_content = response.text  # Get the HTML as text
        soup = BeautifulSoup(biaoqian, "html.parser")

        # Find all <source> tags and extract the URLs
        video_sources = soup.find_all('source')
        video_urls = [source['src'] for source in video_sources if 'src' in source.attrs]
        print('1')
...
#2.07 NjP:/ B@G.vF 09/30 她眼眸中隐约的是孤寂 # 氛围感 https://v.douyin.com/iAJ9TfJV/ 复制此链接，打开Dou音搜索，直接观看视频
# https://www.douyin.com/video/7431114475635297586

#找到图片地址所在
def tuji(biaoqian):
    soup=BeautifulSoup(biaoqian,"html.parser")
    tags = soup.find_all("img",class_="VPaAoQLs") #图片地址所在位置
    #print(tags)
    #print(tags)
    for tag in tags :
        img_src=tag["src"]
        print(img_src)  #输出图片地址
        #down4(img)
